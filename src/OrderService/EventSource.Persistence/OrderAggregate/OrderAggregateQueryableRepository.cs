...
{
    public class OrderAggregateQueryableRepository(IQuerySession querySession)
        : IOrderAggregateQueryableRepository
    {
        public async Task<OrderAggregateEntity?> GetAggregateAsyncSingleAsync(
            Guid id,
        ...
        public async Task<OrderAggregateEntity?> GetAggregateLiveSingleAsync(
            Guid id,
            CancellationToken cancel
        )
        {
